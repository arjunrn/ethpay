...
const (
	ethInWei = float64(1000000000000000000)
)

var (
	ErrCurrentlyUnavailable = errors.New("Currently Unavailable")
)
...
func (r RPCService) MakePayment(ether float64, recepient string) (string, error) {
	...
	log.Debugf("RPC return transaction hash length: %d", len(transactionHash))
	if transactionHash == "" {
		return "", ErrCurrentlyUnavailable
	}
